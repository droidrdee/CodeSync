...
import { addCodesyncSearchExclusion } from './setup/exclude_search';
import { getGitHistory, GitHistoryData } from './panelgit';
import { STASHED_GAIT_STATE_FILE_NAME } from './constants';
import { removeCodesync } from './remove_codesync';
import { initializeCodesync } from './initialize_codesync';
...
export function activate(context: vscode.ExtensionContext) {
   ...
   registerDebugCommand(context);
    ...
    if (fs.existsSync(codesyncFolder)) {
        debug("Codesync folder found");
        context.workspaceState.update('usingCodesync', 'true');
    } else {
        debug("Codesync folder not found");
    ...
    setTimeout(async () => {
        ...
        const filePath = `.codesync/${STASHED_GAIT_STATE_FILE_NAME}`;
    ...
    const excludeSearchCommand = vscode.commands.registerCommand('codesync.excludeSearch', async () => {
        try {
            await addCodesyncSearchExclusion();
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('codesync.showIndividualPanelChat', async (args) => {
